...
const OrderList: React.FC<OrderListProps> = ({ orders, sellers }) => {
  ...
  const handleClearAllOrders = async () => {
    ...
    try {
      // Reautenticação para segurança extra antes de apagar tudo
      ...
      console.error(err);
      setDeleteError("Senha incorreta ou erro de autorização.");
    } finally {
      setIsDeleting(false);
    }
  };
  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-500 pb-10">
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-5">
              ...
              <div>
                ...
                <p className="text-[10px] text-slate-400 font-bold uppercase tracking-[0.3em] mt-2">Painel de controle logístico</p>
              </div>
            </div>
            
            <button 
              onClick={() => setShowClearModal(true)}
              className="flex items-center gap-2 px-4 py-2 bg-red-50 text-red-600 text-[9px] font-black uppercase tracking-widest rounded-xl border border-red-100 hover:bg-red-600 hover:text-white transition-all shadow-sm"
            >
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-4 gap-3">
            ...
            {(searchTerm || statusFilter !== 'all' || sellerFilter !== 'all') && (
              <button 
                ...
                className="px-4 py-3 bg-slate-100 text-slate-500 rounded-2xl text-[9px] font-black uppercase tracking-widest border border-slate-200 hover:bg-slate-200 transition-all flex items-center justify-center gap-2"
              >
      ...
      {/* Modal de Confirmação de Limpeza de Pedidos */}
      {showClearModal && (
        <div className="fixed inset-0 z-[200] flex items-center justify-center p-4 backdrop-blur-md bg-slate-900/40">
          ...
          <div className="relative bg-white w-full max-w-sm rounded-[40px] shadow-2xl border border-slate-200 animate-in zoom-in-95 overflow-hidden">
            <div className="p-8 bg-red-600 text-white flex flex-col items-center gap-4 text-center">
               ...
               <div>
                  <h3 className="text-xs font-black uppercase tracking-[0.2em]">Exclusão em Massa</h3>
                  <p className="text-[10px] font-bold uppercase opacity-70 mt-2">Deseja apagar TODO o banco de dados de pedidos?</p>
               </div>
            ...
            <div className="p-10 space-y-6">
               <div className="space-y-2">
                  <label className="text-[9px] font-black text-slate-400 uppercase tracking-widest ml-1">Senha do Gestor</label>
                  <div className="relative">
                  ...
                  {deleteError && <p className="text-[8px] text-red-600 font-black uppercase tracking-widest ml-1">{deleteError}</p>}
               </div>
               ...
               >
                 {isDeleting ? <Loader2 className="w-4 h-4 animate-spin" /> : <>Limpar Todo Histórico <Trash2 className="w-3.5 h-3.5" /></>}
               </button>
            </div>
          </div>
        </div>
