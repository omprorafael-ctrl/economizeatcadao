...
import { Edit2, Trash2, Search, Plus, Package, FileSearch, X, Tag, Loader2, DollarSign, Layers, AlignLeft, Zap, Filter, Check, Save } from 'lucide-react';
...
const ProductList: React.FC<ProductListProps> = ({ products }) => {
  ...
  const handlePriceUpdate = async (id: string) => {
    ...
    try {
      ...
      alert("Erro ao salvar o novo preço. Verifique o formato.");
    } finally {
      setLoading(false);
    }
  };
                      ...
                      {isEditingPrice ? (
                        <div className="flex items-center gap-2 animate-in slide-in-from-left-2 duration-200">
                          <input 
                            autoFocus
                            type="text"
                            className="w-20 px-2 py-1 bg-white border-2 border-red-500 rounded-lg text-xs font-black text-red-600 outline-none shadow-sm"
                        ...
                        <div 
                          className="flex flex-col cursor-pointer group/price relative"
                          onClick={() => {
                        ...
                        >
                          ...
                          <Edit2 className="w-2.5 h-2.5 text-red-400 absolute -right-4 top-1/2 -translate-y-1/2 opacity-0 group-hover/price:opacity-100 transition-opacity" />
                        </div>
                      )}
                    </td>
                    <td className="px-6 py-4">
